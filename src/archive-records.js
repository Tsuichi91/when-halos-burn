...
const records = [
  {
    ...
    summary:'The foundational public account of the event that created Vesper’s official Hero and Villain roles.', sourceLabel:'OFFICIAL RECORD',
  ...
  },
  {
    ...
    summary:'The official record that preserved Seojun as a symbol while removing the discovery that made him dangerous to HALO.', sourceLabel:'OFFICIAL RECORD',
  ...
  },
  {
    ...
    summary:'HALO’s operational profile of the man it taught Vesper to know as The Architect.', sourceLabel:'OFFICIAL RECORD',
  ...
  },
  {
    ...
    summary:'An accurate service record embedded inside an institutionally distorted understanding of the Blackout.', sourceLabel:'SOURCE RECORD',
    ...
    note:'A document can contain accurate facts and still produce a false conclusion because the system around those facts has already decided what they mean.',
  ...
  },
  {
    ...
    summary:'A suppressed record connecting Taeyun’s failed internal exposure attempt to the disappearance of his mentor.', sourceLabel:'OFFICIAL RECORD',
    ...
    note:'Her absence is one of the reasons Taeyun stopped believing that internal procedure could expose the system safely.',
  ...
  },
  {
    ...
    summary:'A system-level trail linking the Blackout response, concealed evidence and the archive released six years later.', sourceLabel:'SYSTEM RECORD',
    ...
    note:'HALO did not lack evidence. Access to the evidence was controlled.',
  ...
  },
  {
    ...
    summary:'The original operational plan that turns the later relationship into a question of informed choice.', sourceLabel:'ECLIPSE SOURCE',
    ...
    note:'The betrayal in FAULT LINE is not that the HALO evidence is false; it is that Jiwon was denied the information required to choose the relationship with full knowledge of how it began.',
  ...
  },
  {
    ...
    summary:'The surviving fragment Seojun placed in Taeyun’s hands before his death.', sourceLabel:'RECOVERED FRAGMENT',
    ...
    finding:'The fragment independently corroborates the wrongdoing Taeyun and Mirae had been trying to expose. It does not reconstruct the entire Blackout by itself; it connects Seojun directly to the hidden evidence and survives the attempt to erase that connection.',
    note:'Its value comes from corroboration: separate people reached the same institutional truth from different directions.',
  ...
  },
  {
    ...
    summary:'The event record marking the moment the sealed archive stops belonging exclusively to the institution that hid it.', sourceLabel:'EVENT RECORD',
    ...
    finding:'The release exposes the controlled blackout, concealed institutional wrongdoing and false narrative surrounding Seojun and Taeyun. It does not instantly repair Vesper or erase the consequences of anyone’s choices.',
  ...
  },
  {
    ...
    summary:'A record of a city that has received the truth before it has figured out what to do with it.', sourceLabel:'PUBLIC RECORD',
    ...
    finding:'The archive records no instant repair. Vesper remains unsettled, while Jiwon and Taeyun step outside the fixed Hero and Villain roles that once organized the story around them.',
...
const renderRecords = () => {
  ...
  target.innerHTML = `
    <section class="archive-documents">
      <header class="archive-documents__intro"><span>DOCUMENT ARCHIVE / EVIDENCE + SOURCE FILES</span><h3>READ THE RECORD AGAIN.</h3><p>These are the files Vesper used to define what happened — and the recovered evidence that changes what those files mean. A record can contain facts without containing the truth.</p></header>
      <div class="archive-documents__legend" aria-label="Record status legend"><span>TRUST STATE</span><i>VERIFIED</i><i>RECOVERED</i><i>PARTIAL</i><i>ALTERED</i><i>CONTRADICTED</i></div>
      ${items.length ? `<div class="archive-document-grid">${items.map((record) => `
        <button class="archive-document-card" type="button" data-document-id="${record.id}" aria-label="Open ${escapeHtml(record.title)}">
          <span class="archive-document-card__top"><i>${record.id}</i><strong>${record.status}</strong></span><span class="archive-document-card__type">${record.type}</span><span class="archive-document-card__title">${record.title}</span><span class="archive-document-card__summary">${record.summary}</span>
          <span class="archive-document-card__meta"><i><b>DATE</b>${record.date}</i><i><b>SOURCE</b>${record.source}</i><i><b>CLEARANCE</b>${record.clearance}</i></span><span class="archive-document-card__tags">${record.tags.map((tag) => `<i>${tag}</i>`).join('')}</span><span class="archive-document-card__open">OPEN RECORD ↗</span>
        </button>`).join('')}</div>` : '<div class="archive-documents__empty"><span>NO MATCHING DOCUMENT RECORDS</span></div>'}
    </section>`
}
...
const ensureOverlay = () => {
  ...
  overlay.innerHTML = `
    <div class="archive-document-overlay__backdrop" data-document-close></div><article class="archive-document-sheet">
      <button class="archive-document-sheet__close" type="button" data-document-close aria-label="Close record">×</button>
      <header class="archive-document-sheet__header"><div><span data-doc-id></span><i>VESPER ARCHIVE / DOCUMENT RECORD</i></div><strong data-doc-status></strong></header>
      <section class="archive-document-sheet__identity"><span data-doc-type></span><h2 id="archive-document-title" data-doc-title></h2></section>
      <dl class="archive-document-sheet__meta"><div><dt>DATE</dt><dd data-doc-date></dd></div><div><dt>SOURCE</dt><dd data-doc-source></dd></div><div><dt>CLEARANCE</dt><dd data-doc-clearance></dd></div><div><dt>TRUST STATE</dt><dd data-doc-state></dd></div></dl>
      <div class="archive-document-sheet__redactions" aria-hidden="true"><i></i><i></i><i></i></div>
      <section class="archive-document-sheet__comparison"><article><span data-doc-source-label>SOURCE RECORD</span><p data-doc-source-text></p></article><article><span>ARCHIVE FINDING</span><p data-doc-finding></p></article></section>
      <section class="archive-document-sheet__note"><span>RECONSTRUCTION NOTE</span><p data-doc-note></p></section>
      <footer class="archive-document-sheet__footer"><div data-doc-tags></div><a data-doc-chapter hidden>OPEN RELATED CHAPTER ›</a></footer>
    </article>`
...
const openRecord = (record, trigger) => {
  ...
  set('[data-doc-id]',record.id); set('[data-doc-status]',record.status); set('[data-doc-type]',record.type); set('[data-doc-title]',record.title); set('[data-doc-date]',record.date); set('[data-doc-source]',record.source); set('[data-doc-clearance]',record.clearance); set('[data-doc-state]',record.status); set('[data-doc-source-label]',record.sourceLabel); set('[data-doc-source-text]',record.sourceText); set('[data-doc-finding]',record.finding); set('[data-doc-note]',record.note)
  const tags = overlay.querySelector('[data-doc-tags]'); if (tags) tags.innerHTML = record.tags.map((tag) => `<i>${tag}</i>`).join('')
  const chapter = overlay.querySelector('[data-doc-chapter]')
  if (chapter) { if (record.chapter) { chapter.href = getChapterHref(record.chapter); chapter.hidden = false } else { chapter.removeAttribute('href'); chapter.hidden = true } }
...
const closeRecord = () => {
  ...
  overlay.classList.remove('is-open'); document.body.classList.remove('archive-document-open')
...
const ensureIndex = () => {
  ...
  const setIndex = (key,index,label) => {
    ...
    const span = button.querySelector('span'); if (span && span.textContent !== index) span.textContent = index
    if (label) {
      const currentLabel = [...button.childNodes].filter((node) => node.nodeType === Node.TEXT_NODE).map((node) => node.textContent).join('').trim()
  ...
  setIndex('chapters','01'); setIndex('characters','02'); const locations = setIndex('locations','03')
  let recordsButton = nav.querySelector('[data-archive-category="records"]')
  if (!recordsButton) {
    recordsButton = document.createElement('button'); recordsButton.type = 'button'; recordsButton.dataset.archiveCategory = 'records'; recordsButton.innerHTML = '<span>04</span>RECORDS'; locations?.after(recordsButton)
  }
  setIndex('music','05'); setIndex('visuals','06'); setIndex('notes','07','ANNOTATIONS'); setIndex('timeline','08')
  if (recordsButton.dataset.recordsBound !== '1') { recordsButton.dataset.recordsBound = '1'; recordsButton.addEventListener('click',activateRecords) }
  return true
}
...
const activateRecords = () => {
  ...
  const input = searchInput(); if (input) { input.value = ''; input.placeholder = 'Search document, source, status…' }
  renderRecords()
...
const leaveRecords = () => {
  ...
  recordsActive = false; closeRecord()
  const input = searchInput()
  if (input) { input.value = ''; input.placeholder = 'Search title, POV, act…'; requestAnimationFrame(() => input.dispatchEvent(new Event('input',{ bubbles:true }))) }
}
...
const bind = () => {
  ...
  root.addEventListener('click',(event) => {
    ...
    if (category && category.dataset.archiveCategory !== 'records') { leaveRecords(); return }
    const card = event.target.closest('[data-document-id]')
    if (card && currentCategory() === 'records') openRecord(records.find((item) => item.id === card.dataset.documentId),card)
  })
  root.addEventListener('input',(event) => {
    if (event.target !== searchInput() || currentCategory() !== 'records') return
    event.stopImmediatePropagation(); renderRecords()
  },true)
  document.addEventListener('click',(event) => { if (event.target.closest('[data-document-close]')) closeRecord() })
  document.addEventListener('keydown',(event) => { const overlay = document.querySelector('[data-document-overlay]'); if (overlay && !overlay.hidden && event.key === 'Escape') closeRecord() })
  window.addEventListener('pageshow',() => { ensureIndex(); if (currentCategory() === 'records') renderRecords() })
}

const initialize = () => {
  if (ensureIndex()) { bind(); return }
  if (!root) return
  const observer = new MutationObserver(() => { if (!ensureIndex()) return; observer.disconnect(); bind() })
